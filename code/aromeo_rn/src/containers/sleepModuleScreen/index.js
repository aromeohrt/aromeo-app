...
const SleepModuleScreen = (props) => {
  ...
  const [moduleTheme, setModuleTheme] = useState([
    {
      background: constants.Images.SleepBackground,
      blur: true,
      backgroundVideo: constants.Video.SleepScreenVideo,
      music: constants.Music.SleepScreenMusic,
    },
    {
      background: {uri: `${BASE_URL}/reflect-Sleep/Sleep_light_preImg.jpeg`},
      blur: true,
      backgroundVideo: {
        uri: `${BASE_URL}/reflect-Sleep/Sleep_light_portrait.mp4`,
      },
      music: {uri: `${BASE_URL}/reflect-Sleep/Sleep_light_sound.mp3`},
    },
  ]);
  ...
  const handleAppStateChange = async (currentAppState) => {
    if(Platform.OS=="android"){
      ...
      if (currentAppState == 'active') {
        // console.warn(deviceIdRef.current,"Sleep")
  ...
  useEffect(() => {
    musicTime();
    AsyncStorage.getItem('sleepTimer').then((asyncData) => {
      if (asyncData) {
        props.saveSleepTimer(parseInt(asyncData));
  ...
  const musicTime = () => {
    setMusicTimes(props.sleepTimer);
    setCounterTime(props.sleepTimer * 60);
  };
  ...
  return (
    <View style={styles.Flex1}>
      ...
      <ImageBackground
        source={constants.Images.SleepBackground}
        ...
        <SelectTimer
          backgroundImage={
            activeSlide === 0
              ? constants.Images.SleepBackground
              : {uri: `${BASE_URL}/reflect-Sleep/Sleep_light_preImg.jpeg`}
          }
          modalVisible={timerModalVisible}
          closeModal={() => setTimerModalVisible(false)}
          timer={musicTimes}
          saveTimer={props.saveSleepTimer}
          theme="sleepTimer"
        />
        ...
        {toast && stopPressed && (
          <Toast
            headingText="Sleep Session Finished!"
            message={`You have finished ${musicTimes} min of Sleep Session.`}
            customColor="#ffffff"
...
export default connect(
  (state) => ({
    sleepTimer: state.app.sleepTimer,
